...
Logical component in a VPC that represents a **virtual network card (virtual NIC)** attached to an EC2 instance.  
**IP addresses and security settings belong to the ENI, not directly to the EC2 instance.**

An ENI can have the following attributes (SAA‑C03 keywords):
- **`primary private IPv4`**, and **one or more `secondary private IPv4` addresses**
- **`one Elastic IP (IPv4) per private IPv4`** (Elastic IP is *mapped* to a private IP on the ENI)
- **optional public IPv4** (via auto‑assign public IP or Elastic IP mapping)
- **one or more `security groups`**
- a **MAC address**

You can:
- **create ENIs independently** of EC2 instances
- **attach / detach / move ENIs “on the fly” between EC2 instances in the *same AZ*** for **failover**  
  - IP addresses and security groups follow the ENI → **“swap the server, keep the IP”**

Important exam facts:
- **Each EC2 instance must have a primary ENI** (created automatically if you don’t specify one)
- **ENI is bound to a specific Availability Zone (AZ)** → **cannot move an ENI across AZs**
- **Multiple ENIs per instance** allow **separate subnets / security groups / traffic types** (e.g. public traffic vs. management traffic)

## EC2 instance lifecycle – stop, terminate, hibernate

### Stop
- **Instance state**: EC2 instance is **shut down**, but **not deleted**.
- **Root EBS volume**: **kept** (default) – data on the root EBS volume is preserved.
- **Additional EBS volumes**: **kept**, unless you explicitly configured “Delete on termination” (stop does *not* delete them).
- **Instance store volumes** (ephemeral storage): **data is lost** when the instance stops.
- **Networking**:
  - **private IP (on the ENI)**: **preserved**
  - **auto‑assign public IP**: **released and may change** on next start
  - **Elastic IP (if attached)**: **stays associated** and does not change
- **Billing**: you **stop paying for compute**, but still pay for **EBS storage**.

On **start after stop**:
- The **OS boots** again.
- By default, **EC2 User Data runs only on the first boot** (unless you explicitly configure it to run on every boot).
- Then your **application starts**, caches warm up, etc. → can **take time**.

### Terminate
- **Instance state**: EC2 instance is **deleted**.
- **Root EBS volume**: by default **deleted**, unless you disabled “Delete on termination”.
- **Additional EBS volumes**: behavior depends on the **Delete on termination** flag per volume.
- **ENI / IP addresses**: ENIs attached to the instance are **detached and deleted if they were created with the instance**; any **auto public IP is released**, Elastic IPs are **disassociated but not deleted**.
- **Billing**: you **stop paying for compute**; you also **stop paying for any EBS volumes that were deleted**, but keep paying for any volumes you chose to retain.

### Hibernate
- **Goal**: **preserve the in‑memory (RAM) state** so that the instance can **resume much faster** than a normal stop/start.
- **What happens**:
  - The **RAM contents are written to the root EBS volume** (hibernation file).
  - The **instance stops**, but on resume the **OS does not fully reboot**; it **restores RAM from disk**.
- **Requirements** (SAA‑C03):
  - The **root EBS volume must be encrypted**.
  - The root EBS volume must be **large enough to store RAM contents**.
  - Only **supported instance types / OS** combinations can use hibernate (Linux/Windows, not all types).
- **Networking & storage**:
  - **private IP** and **ENI**: **preserved** (same as stop).
  - **auto‑assign public IP**: may be **released** depending on configuration (treat like stop for exam).
  - **EBS volumes**: **kept**, you still pay for storage.

Use cases for **Hibernate** (exam keywords):
- **Long‑running processes** you want to **pause and resume quickly**.
- **Applications with large in‑memory state** (big caches, in‑memory DB) that you don’t want to rebuild.
- **Development / desktop‑like environments** where you want to **resume exactly where you left off**.
...
## EC2 & networking mental model – compare with a physical computer

Use this section as a **memory hook** for the exam. Map each AWS concept to something you already know from a **desktop PC and office network**.

| AWS concept | Physical world analogy | Notes / exam tips |
|------------|------------------------|--------------------|
| **`EC2 instance`** | **Bare desktop tower without disks or network card** | Compute only (CPU/RAM). Needs **EBS (disk)** and **ENI (network card)** to be useful. |
| **`EBS volume`** | **SSD/HDD you plug into your PC** | Persistent storage. Survives **stop/start**; may be deleted on **terminate** depending on settings. Can be **detached/reattached** to other EC2s in the same AZ. |
| **`EC2 instance store`** | **Temporary disk soldered on the motherboard** | Very **fast**, but **data lost** on stop/terminate. Use for **caches / scratch data** only. |
| **`VPC`** | **Your company’s private office network / building** | Isolated virtual network in AWS. Contains **subnets, route tables, IGWs, NAT, etc.** |
| **`Subnet`** | **A floor or VLAN inside the office network** | Sub‑network inside the VPC. Often used as **public / private** zones. |
| **`Security Group (SG)`** | **Host firewall / list of allowed doors and ports on the PC** | **Stateful**, only “allow” rules. Attached to **ENIs/instances**; controls **which traffic can reach the instance**. |
| **`Network ACL (NACL)`** | **Building entrance firewall for each subnet** | **Stateless**, evaluated at subnet boundary. Allows **allow + deny** rules; affects all resources in the subnet. |
| **`ENI (Elastic Network Interface)`** | **LAN / Wi‑Fi card you plug into the desktop** | Virtual NIC. **Owns IPs, MAC address, security groups**. EC2 talks to the network only via ENIs. Can have **multiple ENIs per EC2**. |
| **`Private IP`** | **192.168.x.x internal LAN IP on your home/office PC** | Only reachable **inside the VPC/network**. Stays with the **ENI**, usually does **not change** on stop/start. |
| **`Public IP` (auto‑assign)** | **Temporary public address from your ISP** | Changes when you **stop/start** the instance. Also associated with the **ENI**, but **not guaranteed to be stable**. |
| **`Elastic IP (EIP)`** | **Static public IP block you rent from your ISP** | Your own **fixed public IPv4**. You can **remap** it to another ENI/instance for **failover**. Limited quota; overuse can be a design smell. |
| **`Elastic Load Balancer (ALB/NLB)`** | **Reception desk / load distributor in front of many PCs** | Receives traffic and **forwards to multiple EC2s**. Used for **HA + scaling**. Clients talk to the **LB DNS name**, not directly to instances. |
| **`Auto Scaling Group (ASG)`** | **Auto‑cloning robot that adds/removes desktops as needed** | Automatically **launches/terminates EC2s** based on load/health. Often placed **behind a Load Balancer**. |
| **`IAM role for EC2`** | **Company badge / ID card that defines what systems you can access** | Grants the instance permissions to call **AWS APIs (S3, DynamoDB, etc.)** **without access keys**. Attached at instance level, not to users. |
| **`Placement group`** | **How you arrange desktops in racks and rooms** | **Cluster** = close together (max performance, one big failure domain). **Spread** = all separated (max isolation). **Partition** = grouped zones (for big distributed systems like HDFS). |

> **Exam memory trick**  
> - Think of **EC2** as a **bare desktop**, **EBS** as its **disk**, **ENI** as its **LAN/Wi‑Fi card**, **Security Group** as the **host firewall**, and **VPC/Subnets** as the **office network and floors**.  
> - When you read a question, quickly map it to this physical picture first; then pick the AWS service that plays that role.